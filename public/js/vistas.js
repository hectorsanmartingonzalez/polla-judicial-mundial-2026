...
export function vInicio() {
  ...
  tabla se actualiza sola.</p>
  <hr class="doblelinea">
  <div class="eyebrow">Comparecencia</div>`;

  if (S.cargando) {
    h += skel(50) + skel(50) + skel(50, "70%");
    return h;
  }
  ...
  if (lista.length) {
    h += `<p class="parrafo" style="margin-top:0">¿Ya estás inscrito? Toca tu nombre:</p>`;
    lista.forEach((u, i) => {
      h += `<button class="persona" data-act="soy" data-id="${esc(u.id)}" type="button"
        style="animation-delay:${i * 0.04}s">
        <span>${esc(u.nombre)} ${esc(u.apellido)}</span>
        <small>${predsCount(u.preds || {})}/72</small></button>`;
    });
  } else {
    h += `<p class="vacio">Aún no hay comparecientes.<br>Sé el primero en inscribirte.</p>`;
  }
  h += `<button class="btn violeta" style="margin-top:10px" data-act="reg-on" type="button">
    Inscribirme en la polla</button>
  <footer class="pie">Expediente digital de uso amistoso · v1.0</footer>`;
  return h;
}
...
export function vPred() {
  ...
  if (yo.submittedAt) {
    ...
    <div style="display:flex;gap:8px;margin-top:12px">
      <button class="btn fantasma" style="flex:1" type="button" data-act="copiar">Copiar</button>
      <button class="btn" style="flex:2" type="button" data-act="correo">Enviar copia ✉</button>
    </div>`;
  }
  h += `</div>`;
  return h;
...
export function vTabla() {
  ...
  if (!filas.length) h += `<p class="vacio">Sin participantes inscritos todavía.</p>`;
  filas.forEach((f, i) => {
    h += `<div class="fila${i === 0 && f.total > 0 ? " lider" : ""}" style="animation-delay:${i * 0.045}s">
  ...
  h += `<p class="parrafo" style="font-size:12px;margin-top:14px">Cuota ${clp(CUOTA)} — se recibe
  hasta el día de la final (${FECHA_LIMITE_PAGO}). El pozo se reparte por mitades: premio del
  campeón y fondo de la fiesta.</p>`;
  return h;
}
